 ...
 */
import type { MascotType, MascotWeightVector, QuizOption, QuizQuestion } from '@erciyuan/types'

/**
 * 工具：把仅含部分 mascot_type 的对象补全为完整 7 类权重，未指定的填 0。
 * 让后续 49 个权重的字面量写起来更紧凑而不丢类型安全。
 */
function w(partial: Partial<Record<MascotType, number>>): MascotWeightVector {
  return {
...
function opts(
  list: Array<[QuizOption['key'], string, MascotWeightVector]>,
): QuizQuestion['options'] {
  if (list.length !== 7) {
    throw new Error(`[mock-town/quizzes] 每题必须恰好 7 个选项，收到 ${list.length}`)
  }
  const made = list.map(([key, text, weights]): QuizOption => ({ key, text, weights }))
  return made as unknown as QuizQuestion['options']
}
