 ...
 */
import type { MockRuntimeHandle, ServerMsg, StartRuntimeOptions } from '@erciyuan/types'

import { timeline } from './timeline.js'

/** setTimeout / setInterval 句柄（兼容 Node `Timeout` 与 Browser `number`）。 */
 ...
 * @returns 控制句柄
 */
export function startRuntime(opts: StartRuntimeOptions): MockRuntimeHandle {
   ...
   * 并通过 onError 把异常透给上层。
   */
  function schedule(fromSeconds: number): void {
    if (stopped || fallbackInterval !== null) return
    ...
    try {
      ...
      for (const evt of remaining) {
        ...
        const t = setTimeout(() => {
          ...
          if (stopped || fallbackInterval !== null) return
          try {
            onEvent(evt.msg satisfies ServerMsg)
          } catch {
   ...
   */
  function forceFallback(reason: string = 'manual'): void {
    if (stopped) return
    if (fallbackInterval !== null) return // 已经在兜底里了，幂等
 ...
 */
export function getStaticTimeline(): typeof timeline {
