...
// ─── Re-export every public type from @erciyuan/types ──────────────────────
//
// 让消费者只需 `import { Character, ServerMsg, ... } from '@erciyuan/mock-town'`
// 即可拿到所有 schema，无需感知 types 包的存在。
export type {
  Action,
  AssetCategory,
  AssetEntry,
...
// Re-export the locally-defined NpcRecord schema for consumers that need to
// type their own NPC arrays (Sync_Service NPC spawn path, dashboard etc.).
...
} from './mascot-resolver.js'
export { npcs } from './npcs.js'

// ─── Aggregate API for one-stop consumption ────────────────────────────────
import type {
  MascotProfile,
  MascotType,
  MascotWeightVector,
 ...
 */
export const mockTown: MockTownAPI = {
  videos,
  conventions,
  companions,
  quizzes,
